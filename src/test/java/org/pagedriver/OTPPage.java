...
public class OTPPage extends BaseDriver {
    WebDriver driver;

    ExtentTest test;

    public OTPPage(ExtentTest test) {
        PageFactory.initElements(PageDriver.getCurrentDriver(), this);
        this.test = test;
    }

    @FindBy(xpath="//*[@id=\"pin-check\"]")
    WebElement otp;
    ...
    public void otpPageGet() throws InterruptedException, IOException {
        Actions action = new Actions(PageDriver.getCurrentDriver());
        try {
            test.info("Hover on bookInformation");
            if (otp.isDisplayed()){
                action.moveToElement(otp).build().perform();
                Thread.sleep(3000);
                otp.sendKeys("");
                Thread.sleep(3000);
